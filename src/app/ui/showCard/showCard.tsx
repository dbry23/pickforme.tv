...
export default function ShowCard({ show }: { show: Show }) {
  const [isInLibrary, setIsInLibrary] = useState<boolean>(() => isShowInLibrary(show.id));

  const tmdbImgUrlBase = 'https://image.tmdb.org/t/p/w780';
  const backgroundStyle =
    show.backdrop_path ? { backgroundImage: `url(${tmdbImgUrlBase}${show.backdrop_path})` } : {};

  const toggleText = isInLibrary ? 'Remove' : 'Add';
  const toggleTextClass = isInLibrary ? styles.toggleTextRemove : styles.toggleTextAdd;
  ...
  return (
    <div className={styles.card} style={backgroundStyle} onClick={handleClick} role="button">
